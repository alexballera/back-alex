  ...
  private
    ...
    def set_project
      if Project.exists?(params[:id])
        ...
        respond_to do |format|
          format.html { redirect_to users_path, :alert => 'El proyecto no existe' }
        end
      end
    end
    ...
    def project_params
      params.require(:project).permit(:title, :description, :user_id, :image, :urlgit, :urlweb, :urlimg, :category_ids => [])
    end
end
